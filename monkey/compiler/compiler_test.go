...
func testConstants(
	...
	for i, constants := range expected {
		...
		case []code.Instructions:
			...
			if err != nil {
				return fmt.Errorf("constant %d testInstructions faield %s", i, err)
			}
		}
	}
...
func TestIntergerArithemetic(t *testing.T) {
...
func TestBooleanExperessions(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             "true",
			expectedConstants: []interface{}{},
...
func TestComplerScopes(t *testing.T) {
	compiler := New()
	...
	globalSybolTable := compiler.symbolTable
	...
	if compiler.symbolTable.Outer != globalSybolTable {
	...
	if compiler.symbolTable != globalSybolTable {
		t.Errorf("compiler did not enclose symbolTable")
	}

	if compiler.symbolTable.Outer != nil {
		t.Errorf("compiler modified global symbol table incorrecently")
	}
