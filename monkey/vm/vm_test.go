...
func testExpectedObject(
	...
	case string:
		...
		if err != nil {
			t.Errorf("testsStringObject failed: %s", err)
		}
	...
	case []int:
		...
		for i, expectedElem := range expected {
			err := testIntergerObject(int64(expectedElem), array.Elements[i])
			if err != nil {
...
func TestFunctionWitReturnStatement(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
